 ...
 function load_location_address(select){
	...
	$.getJSON('elements/ajax.php', {action:'getlocationaddress', id_city:select.value}, function(resultList){
		...
		$.each(resultList, function(i){
			if(i==0) {
				resultSelect.append('<option value="' + i + '" disabled selected="selected">' + this + '</option>');
			}
			else
				resultSelect.append('<option value="' + i + '">' + this + '</option>');
		});
...
function load_mvz(select){
	var resultSelect = $('select[name="id_mvz"]');
	resultSelect.attr('disabled', 'disabled'); // делаем список не активным
	// послыаем AJAX запрос, который вернёт список для выбранной области
	$.getJSON('elements/ajax.php', {action:'get_mvz', id_rck:select.value}, function(resultList){
		resultSelect.html(''); // очищаем список
		// заполняем список  новыми пришедшими данными
		$.each(resultList, function(i){
			if(i==0) {
...
function load_data(select,names){
	...
	resultSelect.attr('disabled', 'disabled'); // делаем список не активным
	// послыаем AJAX запрос, который вернёт список для выбранной области
	$.getJSON('elements/ajax.php', {action:'get_department', id_department:select.value}, function(resultList){
		resultSelect.html(''); // очищаем список
		$('span[id |= "select2-'+names+'"]').html('');
		if(names == 'id_department_2'){
			$('span[id |= "select2-id_department_3"]').html('');
			$('span[id |= "select2-id_department_4"]').html('');
			$('select[name="id_department_3"]').html('');
			$('select[name="id_department_4"]').html('');
		}
		if(names == 'id_dep_3'){
			$('span[id |= "select2-id_department_4"]').html('');
			$('select[name="id_dep_4"]').html('');
			//$('select[name="id_department_4"]').append('<option value="0" disabled selected="selected">Выберите</option>');
		}
		// заполняем список  новыми пришедшими данными
		$.each(resultList, function(i){
			if(i==0) {
